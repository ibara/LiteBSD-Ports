...
typedef struct {
	union{
		...
		Ehdr elfhdr32;			/* elf.h */
		struct mipsexec pmips;	/* bootexec.h */
...
mipsboot(int fd, Fhdr *fp, ExecHdr *hp)
{
	USED(fd);
	fp->type = FMIPSB;
	switch(hp->e.pmips.amagic) {
	default:
	case 0407:	/* some kind of mips */
		settext(fp, (u32int)hp->e.pmips.mentry, (u32int)hp->e.pmips.text_start,
			hp->e.pmips.tsize, sizeof(struct mipsexec)+4);
		setdata(fp, (u32int)hp->e.pmips.data_start, hp->e.pmips.dsize,
			fp->txtoff+hp->e.pmips.tsize, hp->e.pmips.bsize);
		break;
	case 0413:	/* some kind of mips */
		settext(fp, (u32int)hp->e.pmips.mentry, (u32int)hp->e.pmips.text_start,
			hp->e.pmips.tsize, 0);
		setdata(fp, (u32int)hp->e.pmips.data_start, hp->e.pmips.dsize,
			hp->e.pmips.tsize, hp->e.pmips.bsize);
		break;
	}
	setsym(fp, hp->e.pmips.nsyms, 0, hp->e.pmips.pcsize, hp->e.pmips.symptr);
	fp->hdrsz = 0;			/* header stripped */
	return 1;
}
